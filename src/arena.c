#define PMALLOC_ARENA_C_
#include "pmalloc/internal/pmalloc_internal.h"

/******************************************************************************/
/* Data. */

const char PMEMPATH[2][50] = {"/mnt/pmem1/pmalloc_files/", "/mnt/pmem/pmalloc_files/"};
...
void arena_slab_dalloc(tcache_t *tcache, arena_t *arena, extent_t *slab)
{
	...
	pthread_mutex_lock(&arena->log_lock);
	add_log(arena, &arena->log, FREE_LOG, (uint64_t)log, (uint64_t)NULL, true);
	pthread_mutex_unlock(&arena->log_lock);
}
