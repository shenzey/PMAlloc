...
#ifdef SLAB_MORPHING
    if (arena->morphing_len > 0 && arena->morphing_len > 1.0 * arena->LRU_len * MORHPING_LIST_SEARCH_THRESHOLD)
    {
...
void migrate_putback(arena_t *arena, tcache_t *tcache, vslab_t *vslab, void *ptr)
{
    sizeclass_t sc = vslab->sc;
    int sc_idx = get_sizeclass_id_by_size(sc.bitsize);
#ifdef SLAB_MORPHING
